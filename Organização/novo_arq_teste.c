...
bancoPecas campo[28];     // Variável GLOBAL do tipo bancoPecas, para armazenar as informções do campo, como tamanho do campo e quais peças estão no campo
...
void gerarCampo(){
    for(int i=0;i<28;i++){
        campo[i].pecas[i].esquerda = 7;
        campo[i].pecas[i].direita = 7;
    }
}
...
if(daPraJogar == 0){
    ...
    if(qualLado == 0){ // Jogar peça na ESQUERDA do campo
        ...
        campo[0].pecas[0].esquerda = jogadores[idJogador-1].maoJogador[idPeca].esquerda;
        campo[0].pecas[0].direita = jogadores[idJogador-1].maoJogador[idPeca].direita;
    }else if (qualLado == 1){ // Jogar peça na DIREITA
    int contador = 0;
        for(int i=0;i<28;i++){
            if (campo[i].pecas[i].esquerda != 7 || campo[i].pecas[i].direita != 7){
                contador++;
            }
        }
    ...
    if(qualLado == 0){ // Jogar peça na ESQUERDA do campo
        for(int i=0;i<27;i++){ // Laço que copia todas as peças para uma posição a frente
            campo[i].pecas[i].esquerda = campo[i+1].pecas[i+1].esquerda;
            campo[i].pecas[i].direita = campo[i+1].pecas[i+1].direita;
            }
        campo[0].pecas[0].esquerda = jogadores[idJogador-1].maoJogador[idPeca].direita;
        campo[0].pecas[0].direita = jogadores[idJogador-1].maoJogador[idPeca].esquerda;
    }else if (qualLado == 1){ // Jogar peça na DIREITA
    int contador = 0;
        for(int i=0;i<28;i++){
            if (campo[i].pecas[i].esquerda != 7 || campo[i].pecas[i].direita != 7){
        ...
        campo[contador].pecas[contador].esquerda = jogadores[idJogador-1].maoJogador[idPeca].direita;
        campo[contador].pecas[contador].direita = jogadores[idJogador-1].maoJogador[idPeca].esquerda;
    }
}
...
if(qualLado == 0){ // Jogar peça na ESQUERDA
    if(campo[0].pecas[0].esquerda == jogadores[idJogador-1].maoJogador[idPeca].direita){
        return 1;
    }else if (campo[0].pecas[0].esquerda == jogadores[idJogador-1].maoJogador[idPeca].esquerda){
        return 2;
    }else{
        return 0;
    }
    ...
    if(campo[contador].pecas[contador].direita == jogadores[idJogador-1].maoJogador[idPeca].esquerda){
        return 1;
    }else if (campo[contador].pecas[contador].direita == jogadores[idJogador-1].maoJogador[idPeca].direita){
        return 2;
    }else{
        return 0;
    }
...
void imprimirCampo(){
    // Função responsável por imprimir na tela as peças atuais no campo
     for (int i = 0; i < 28; i++) {                                     // Laço de repetição que itera todos os elemento do vetor campo
         printf("[%i][%i]", campo[i].pecas[i].esquerda, campo[i].pecas[i].direita);  // Imprime as peças presentes no campo até então
     }
}
...
void imprimirCampof(){
    for (int i = 0; i < 28; i++){
        if(campo[i].pecas[i].esquerda != 7 || campo[i].pecas[i].direita != 7){
            printf("[%i][%i]",campo[i].pecas[i].esquerda,campo[i].pecas[i].direita);
        }
    }
}
...
void pecaInicial(){
    // Função responsável por analisar as mãos dos jogadores, guardar a peça de maior valor de cada um e compara-lás, retornando um valor correspondente
} // Função esta, é fundamental para determinar a primeira peça jogada no campo

int main(){
    srand(time(NULL));
    jogadores[0].maoJogador = malloc(7 * sizeof(pecaDomino));
    jogadores[1].maoJogador = malloc(7 * sizeof(pecaDomino));
    // campo.pecas = malloc(1 * sizeof(pecaDomino));
    ...
    // campo.pecas[0].esquerda = 6; //Teste
    // campo.pecas[0].direita = 6;  //Teste
    // campo.tam++;

    campo[0].pecas[0].esquerda = 6;
    campo[0].pecas[0].direita = 6;
